...
if __name__ == '__main__':
    ...
    if not os.path.exists('./_temp'):
        os.makedirs('./_temp')
    if not os.path.exists('./_temp/pred'):
        os.makedirs('./_temp/pred')
    if not os.path.exists('./_temp/original'):
        os.makedirs('./_temp/original')

    # load saved model
    file = open('./models/model.json', 'r')
    ...
    for index in tqdm(range(0, mix_wav_mag.shape[1], patch_size)):
            ...
            save_audio("./_temp/original/" + str(index//patch_size)+".wav", mix_mag, mix_phase)

    filenames = find_files("./_temp/pred",ext="wav")
    ...
    filenames = find_files("./_temp/original",ext="wav")
    combined = AudioSegment.empty()
    for filename in filenames:
        audio = AudioSegment.from_wav(filename)
        combined += audio
